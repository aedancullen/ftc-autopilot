...
public class AutopilotTrackerEncMec extends AutopilotTracker{
    ...
    public AutopilotTrackerEncMec(DcMotor fl, DcMotor fr, DcMotor bl, DcMotor br, double ticksPerUnit, BNO055IMU imu, int nSubsteps) {
        this.fl = fl;
        this.fr = fr;
        this.bl = bl;
        this.br = br;
    ...
    public void update() {
        ...
        long ticksFl = fl.getCurrentPosition();
        long ticksFr = fr.getCurrentPosition();
        long ticksBl = bl.getCurrentPosition();
        long ticksBr = br.getCurrentPosition();

        double yval = (
                ((double)(ticksFl - flenc) / ticksPerUnit) + ((double)(ticksFr - frenc) / ticksPerUnit)
                + ((double)(ticksBl - blenc) / ticksPerUnit) + ((double)(ticksBr - brenc) / ticksPerUnit)
        ) / 4.0;


        // track strafe - SubLeft is fl minus bl, SubRight is br minus fr. See drive() in MarvMk8CCommon to understand.
        ...
        double xval = (xvalSubLeft + xvalSubRight) / 2.0;
        ...
        // so the xval has now been incorporated here, in comparison to the older AutopilotTrackerEnc.
